...
Transfuser 的缺點: IL 太侷限在模擬 Autopilot 的行為, 導致generalism不好, 無法適應新環境, 且若 Autopilot 行為本身就有缺失的情況下, NN更難做出好的決策, ex:Autopilot 在撞到人後會停止, 或是在某些紅綠燈scenario下會停止, 這會導致 NN 預測時會常常中途停下來。 
```
...
AgentBlockedTest: This test will fail if the actor has had its linear velocity lower than a specific value for a specific amount of time  
