...
def get_hparams(playing=False):
    ...
    hparams = None
    if os.path.isfile('C:\\Users\\Administrator\\Carla-RL-master (1)\\Carla-RL-master\\checkpoint\\hparams.json'):

        # If JSON file exists, load it
        with open('C:\\Users\\Administrator\\Carla-RL-master (1)\\Carla-RL-master\\checkpoint\\hparams.json', encoding='utf-8') as f:
            hparams = json.load(f)
...
def get_hparams_abs_addr(dir, playing=False):

    hparams = None
    if os.path.isfile(dir+'\\'+'checkpoint\\hparams.json'):

        # If JSON file exists, load it
        with open(dir+'\\'+'checkpoint\\hparams.json', encoding='utf-8') as f:
            hparams = json.load(f)
