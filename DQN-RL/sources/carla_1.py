...
try:
    sys.path.append(glob.glob('D:\git\carla\PythonAPI\carla\dist\carla-*%d.%d-%s.egg' % (
...
class CarlaEnv:
    ...
    def reset(self):
        ...
        # Get the blueprint for the camera
        self.ss_cam = self.world.get_blueprint_library().find('sensor.camera.semantic_segmentation')
        # Set sensor resolution and field of view
        self.ss_cam.set_attribute('image_size_x', f'{self.im_width}')
        self.ss_cam.set_attribute('image_size_y', f'{self.im_height}')
        self.ss_cam.set_attribute('fov', '110')

        # Set camera sensor relative to a car
        transform_ss = carla.Transform(carla.Location(x=2.5, z=0.7))

        # Attach camera sensor to a car, so it will keep relative difference to it
        self.sensor_ss = self.world.spawn_actor(self.ss_cam, transform_ss, attach_to=self.vehicle)

        # Register a callback called every time sensor sends a new data
        self.sensor_ss.listen(self._process_img)

        # Add camera sensor to the list of actors
        self.actor_list.append(self.sensor_ss)
    ...
    def step(self, action):
        ...
        if time.time() > self.last_cam_update + 1:
            print('Missing updates from Carla')
            ...
            while True:
                ...
                try:
                    ...
                    if self.next_world_reload and time.time() > self.next_world_reload:
                        ...
                        map_choice = settings.CARLA_HOSTS[0][2]
                        ...
                        while self.client.get_world().get_map().name != map_choice:
                            retries += 1
                            if retries >= 600:
                                print('Timeout when waiting for new map to be fully loaded')
